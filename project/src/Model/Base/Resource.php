...
/**
 * Base class that represents a row from the 'box_resource' table.
 *
 *
 *
...
abstract class Resource implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Box\\Model\\Map\\ResourceTableMap';
    ...
    /**
     * Initializes internal state of Box\Model\Base\Resource object.
     */
     ...
     * @return $this|\Box\Model\Resource The current object (for fluent API support)
     */
    ...
    public function setParameter($v)
    {
        ...
        if ($this->parameter !== $v) {
            $this->parameter = $v;
            $this->modifiedColumns[ResourceTableMap::COL_PARAMETER] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ResourceTableMap::translateFieldName('Parameter', TableMap::TYPE_PHPNAME, $indexType)];
            $this->parameter = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ResourceTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            $this->value = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ResourceTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : ResourceTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 6; // 6 = ResourceTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Box\\Model\\Resource'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('box_resource_id_seq')");
        ...
        if ($this->isColumnModified(ResourceTableMap::COL_PARAMETER)) {
        ...
        $sql = sprintf(
            'INSERT INTO box_resource (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Box\Model\Resource
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Box\Model\Resource The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ResourceTableMap::COL_PARAMETER)) {
            $criteria->add(ResourceTableMap::COL_PARAMETER, $this->parameter);
        }
        if ($this->isColumnModified(ResourceTableMap::COL_VALUE)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Box\Model\Resource (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Box\Model\Resource Clone of current object.
     * @throws PropelException
     */
